...
class CampingArgParser(argparse.ArgumentParser):

    def __init__(self):
        super().__init__()
        self.add_argument("--debug", "-d", action="store_true", help="Debug log level")
        self.add_argument(
            "--start-date", required=True, help="Start date [YYYY-MM-DD]", type=self.TypeConverter.date
        )
        self.add_argument(
            "--end-date",
            required=True,
    ...
    def _validate_args(args):
        if len(args.parks) > 1 and len(args.campsite_ids) > 0:
            raise CampingArgParser.ArgumentCombinationError("--campsite-ids can only be used with a single park ID.")
